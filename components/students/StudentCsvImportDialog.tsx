...
type ImportRowFilter = 'all' | 'create' | 'update' | 'error';
...
export function StudentCsvImportDialog({ open, onClose, onImported }: StudentCsvImportDialogProps) {
  ...
  const filteredRows = useMemo(() => {
    if (!analysis) return [];
    if (rowFilter === 'all') return analysis.rows;
    return analysis.rows.filter((r) => r.action === rowFilter);
  }, [analysis, rowFilter]);
  ...
  const actionColor = (action: string) => {
    ...
    if (action === 'skip') return 'bg-slate-100 text-slate-700 border-slate-200';
              ...
              <>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
                  <button
                    ...
                    className={cn(
                      'rounded-lg border border-slate-200 bg-slate-50 p-3 text-center transition hover:bg-slate-100 sm:p-4',
                      rowFilter === 'all' && 'ring-2 ring-slate-700 ring-offset-1',
                    )}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setRowFilter('create')}
                    className={cn(
                      'rounded-lg border border-green-100 bg-green-50 p-3 text-center transition hover:bg-green-100/80 sm:p-4',
                      rowFilter === 'create' && 'ring-2 ring-green-700 ring-offset-1',
                    )}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setRowFilter('update')}
                    className={cn(
                      'rounded-lg border border-blue-100 bg-blue-50 p-3 text-center transition hover:bg-blue-100/80 sm:p-4',
                      rowFilter === 'update' && 'ring-2 ring-blue-700 ring-offset-1',
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      'rounded-lg border border-red-100 bg-red-50 p-3 text-center transition hover:bg-red-100/80 sm:p-4',
                      rowFilter === 'error' && 'ring-2 ring-red-700 ring-offset-1',
                    )}
                  >
                ...
                <p className="text-xs text-slate-500">
                  Tap a summary card to filter the table below. Showing{' '}
                  <span className="font-medium text-slate-700">{filteredRows.length}</span> of{' '}
                  {analysis.rows.length} ({filterLabel.toLowerCase()}).
                </p>
                    ...
                    <div className="max-h-64 overflow-auto sm:max-h-72 md:max-h-[min(42vh,24rem)] lg:max-h-[min(48vh,28rem)]">
                      {filteredRows.length === 0 ? (
                        <p className="px-4 py-8 text-center text-sm text-slate-500">
                          No {filterLabel.toLowerCase()} rows in this file.
                        </p>
                      ) : (
