...
import { ProfileImageEnlarge, type ProfileQuickViewDetails } from '@/components/ProfileImageEnlarge';
...
import { formatClassFilterLabel, formatClassSection } from '@/lib/grade-class-options';
...
function studentQuickView(student: Student, compact?: boolean): ProfileQuickViewDetails {
  const classLabel = formatClassSection(student.grade, student.section);
  const gradeLine = [student.grade, classLabel].filter(Boolean).join(' · ');

  return {
    identifier: student.admissionNumber,
    subtitle: gradeLine || undefined,
    initialsLine: compact ? undefined : student.nameWithInitials?.trim() || undefined,
    badges: [{ label: student.status, variant: statusVariants[student.status] }],
    phone: student.phone || student.parentPhone,
    email: student.email || student.parentEmail,
    profileHref: `/dashboard/students/${student.id}`,
    accent: 'blue',
  };
}
...
export function StudentsDirectory({
  ...
  const clearFilters = () => {
    ...
    onClassFilterChange(FILTER_ALL);
  };

  const classFilterRow =
    classFilterOptions.length > 0 ? (
      <div className="flex items-center gap-2 overflow-x-auto pb-0.5 [scrollbar-width:thin]">
        ...
        <FilterPill active={classFilter === FILTER_ALL} onClick={() => onClassFilterChange(FILTER_ALL)}>
          All
        </FilterPill>
        {classFilterOptions.map((c) => (
          <FilterPill
            ...
            onClick={() => onClassFilterChange(classFilter === c ? FILTER_ALL : c)}
          >
            {formatClassFilterLabel(c)}
          </FilterPill>
        ))}
      </div>
    ) : null;
      ...
      {!restricted && (
        <div className="shrink-0 rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm ring-1 ring-black/[0.02] sm:p-4">
          ...
          <div className="mt-3 hidden lg:block">
            ...
            {classFilterRow && <div className="mt-2">{classFilterRow}</div>}
          </div>
        </div>
      )}
      ...
      <div className="directory-panel ring-1 ring-black/[0.02]">
        ...
        <div className="directory-panel-scroll">
          {loading ? (
            ...
            <>
              <ul className="flex flex-col gap-1.5 p-1.5 sm:gap-0 sm:p-0 lg:divide-y lg:divide-slate-100">
                <AnimatePresence mode="popLayout">
                  {visibleStudents.map((s, i) => (
                    <StudentListRow
                      key={s.id}
...
const StudentListRow = memo(function StudentListRow({
  ...
  const classLabel = formatClassSection(s.grade, s.section);
  ...
  return (
    ...
    >
      <div
        className={cn(
          'group rounded-xl border border-slate-100 bg-white shadow-sm transition-all',
          'hover:border-blue-200/70 hover:shadow-md',
          'sm:rounded-none sm:border-0 sm:border-b sm:border-slate-100 sm:bg-transparent sm:shadow-none sm:hover:border-slate-100 sm:hover:shadow-none sm:last:border-b-0',
        )}
      >
        <div className="flex items-start gap-2.5 p-2.5 sm:items-center sm:gap-3 sm:px-4 sm:py-2.5 lg:py-3">
          <ProfileImageEnlarge
            name={s.name}
            profileImageUrl={s.profileImageUrl}
            quickView={studentQuickView(s, compact)}
          >
            <StudentProfilePhoto
              name={s.name}
              profileImageUrl={s.profileImageUrl}
              size="md"
              className="h-10 w-10 rounded-lg sm:h-11 sm:w-11 sm:rounded-xl"
            />
          </ProfileImageEnlarge>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-1.5">
              <Link
                ...
                className="min-w-0 flex-1 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40 focus-visible:ring-offset-2"
              >
                <p className="line-clamp-2 text-[13px] font-bold leading-tight text-slate-900 group-hover:text-blue-800 sm:text-sm sm:leading-snug">
                  {s.name}
                </p>
                {!compact && s.nameWithInitials?.trim() && (
                  <p className="mt-0.5 hidden line-clamp-1 text-[10px] font-medium text-slate-500 md:block">
                    {s.nameWithInitials}
                  </p>
                )}
                <div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                  <span className="font-mono text-[10px] font-semibold text-blue-700 sm:text-[11px]">
                    {s.admissionNumber}
                  </span>
                  ...
                  <Badge
                    ...
                    className="h-4 shrink-0 px-1.5 py-0 text-[9px] capitalize leading-none lg:hidden"
                  >
                    {s.status}
                  </Badge>
                </div>
              ...
              <div
                className="-mr-1 flex shrink-0 items-center lg:hidden"
          ...
          <div
            className="hidden shrink-0 flex-col items-end gap-2 lg:flex"
            onClick={(e) => e.stopPropagation()}
          >
        ...
        <div className="flex items-center justify-between border-t border-slate-100 px-2.5 py-1.5 sm:hidden">
          <Link
            href={`/dashboard/students/${s.id}`}
            className="inline-flex items-center gap-0.5 text-[11px] font-semibold text-blue-700"
          >
            View profile
            <ChevronRight className="h-3 w-3" />
          </Link>
          {contact && (
            <ContactActionLink kind="tel" value={contact} className="text-[11px] font-medium text-slate-600" />
          )}
        </div>
      </div>
...
function StudentGridCard({
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col rounded-xl border border-slate-200/80 bg-white p-3 shadow-sm transition-shadow hover:border-blue-200/60 hover:shadow-md">
        <div className="flex flex-1 flex-col items-center text-center">
          <ProfileImageEnlarge
            name={s.name}
            profileImageUrl={s.profileImageUrl}
            quickView={studentQuickView(s)}
          >
            <StudentProfilePhoto
              name={s.name}
              profileImageUrl={s.profileImageUrl}
          ...
          <Link href={`/dashboard/students/${s.id}`} className="mt-2 flex w-full flex-col items-center text-center">
            <p className="line-clamp-2 text-sm font-bold text-slate-900">{s.name}</p>
            ...
            <p className="mt-1 line-clamp-2 text-[11px] text-slate-600">
              {s.grade}
              {formatClassSection(s.grade, s.section)
                ? ` · ${formatClassSection(s.grade, s.section)}`
                : ''}
            </p>
            <Badge variant={statusVariants[s.status]} className="mt-2 capitalize text-[10px]">
              {s.status}
            </Badge>
          </Link>
        </div>
