...
function staffQuickView(member: Staff, limitedView?: boolean): ProfileQuickViewDetails {
  return {
    identifier: limitedView ? undefined : member.staffId,
    subtitle: limitedView
      ? member.classAndGrade?.trim() || undefined
      : member.designation?.trim() || undefined,
    initialsLine: member.nameWithInitials?.trim() || undefined,
    badges: limitedView
      ? undefined
      : [
          {
            label: member.staffType === 'academic' ? 'Academic' : 'Non-academic',
            variant: member.staffType === 'academic' ? 'default' : 'secondary',
          },
          { label: member.status, variant: statusVariants[member.status] },
        ],
    phone: member.phone,
    email: member.email,
    profileHref: `/dashboard/staff/${member.id}`,
    accent: 'emerald',
  };
}
  ...
  return (
    ...
    >
      ...
      >
        <SelectTrigger
          className={cn(
            'gap-1 font-semibold capitalize shadow-none',
            compact
              ? 'h-5 w-auto min-w-[4.25rem] border-0 px-1.5 text-[9px] ring-1'
              : 'h-7 w-[7.25rem] border-slate-200 bg-white px-2 text-[11px] shadow-sm',
            member.status === 'active' &&
              (compact
                ? 'bg-emerald-50 text-emerald-800 ring-emerald-200/80'
                : 'border-green-200 text-green-800'),
            member.status === 'inactive' &&
              (compact ? 'bg-slate-50 text-slate-600 ring-slate-200' : 'text-slate-600'),
      ...
      >
        <div className="flex items-start gap-2.5 p-2.5 sm:items-center sm:gap-3 sm:px-4 sm:py-2.5 lg:py-3">
          <ProfileImageEnlarge
            ...
            quickView={staffQuickView(s, limitedView)}
          >
            <StaffAvatar
              name={s.name}
              profileImageUrl={s.profileImageUrl}
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-1.5">
              <Link
                ...
                className="min-w-0 flex-1 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 focus-visible:ring-offset-2"
              >
                <p className="line-clamp-2 text-[13px] font-bold leading-tight text-slate-900 group-hover:text-emerald-800 sm:text-sm sm:leading-snug">
                  {s.name}
                </p>
                {s.nameWithInitials?.trim() && (
                  <p
                    className={cn(
                      'mt-0.5 line-clamp-1 text-[10px] font-medium text-slate-500',
                      !limitedView && 'hidden md:block',
                    )}
                  >
                    {s.nameWithInitials}
                  </p>
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                  {limitedView ? (
                    ...
                    <>
                      <span className="font-mono text-[10px] font-semibold text-emerald-700 sm:text-[11px]">
                        {s.staffId}
                      </span>
                      {s.designation?.trim() && (
                        <>
                          <span className="text-slate-300">·</span>
                          <span className="line-clamp-1 text-[10px] text-slate-600 sm:text-[11px]">
                            {s.designation}
                          </span>
                        </>
                      )}
                  ...
                  {!limitedView && (
                    <>
                      <Badge
                        ...
                        className="h-4 shrink-0 px-1.5 py-0 text-[9px] leading-none lg:hidden"
                      >
            ...
            {subjectsPreview && (
              <p className="mt-1 hidden items-center gap-1 text-[11px] text-slate-500 lg:flex">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 px-2.5 py-1.5 sm:hidden">
          <Link
            href={`/dashboard/staff/${s.id}`}
            className="inline-flex items-center gap-0.5 text-[11px] font-semibold text-emerald-700"
          >
            View profile
            <ChevronRight className="h-3 w-3" />
          </Link>
          {s.phone && (
            <ContactActionLink kind="tel" value={s.phone} className="text-[11px] font-medium text-slate-600" />
          )}
        </div>
      </div>
    ...
    >
      <div className="flex h-full flex-col rounded-xl border border-slate-200/80 bg-white p-3 shadow-sm transition-shadow hover:border-emerald-200/60 hover:shadow-md">
        <div className="flex flex-1 flex-col items-center text-center">
          ...
          <Link href={`/dashboard/staff/${s.id}`} className="mt-2 flex w-full flex-col items-center text-center">
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="icon"
        className="touch-target-icon h-9 w-9 text-rose-500 hover:bg-rose-50 hover:text-rose-700"
        title="Delete"
