...
interface TeacherSubjectDetailDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  row: TeacherSubjectAnalysisRow | null;
  results: ExamResult[];
  staff: Staff[];
...
function CountCell({ value, variant }: { value: number; variant: 'below' | 'above' }) {
  const active =
    variant === 'below'
      ? value > 0
      ...
      : value > 0
        ...
        : 'text-slate-400';
  return (
    <span className={cn('inline-block tabular-nums decoration-0', active)}>{value}</span>
  );
}

export function TeacherSubjectDetailDialog({
  open,
  onOpenChange,
  row,
  results,
  staff,
  exam,
}: TeacherSubjectDetailDialogProps) {
  const detail = useMemo(
    () => (row ? buildTeacherSubjectDetail(results, staff, row) : null),
    [row, results, staff],
  );
      ...
      <DialogContent className="flex max-h-[min(92vh,900px)] w-[calc(100vw-1.5rem)] max-w-2xl flex-col gap-0 overflow-hidden p-0 sm:w-full">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-5 py-4 sm:px-6">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <MiniStat label="Students" value={detail.totalStudents} />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
              {SCORE_BANDS.map((band) => (
                <div
                  key={band}
                  className="flex flex-col items-center rounded-lg border border-slate-200 bg-white px-2 py-2"
                >
          ...
          {detail.gradeBreakdown.length > 0 && (
            <div>
              <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-slate-500">
                By exam grade
              </p>
              <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white lg:hidden">
                {detail.gradeBreakdown.map((g) => (
                  <li key={g.grade} className="flex items-center justify-between gap-2 px-3 py-2.5 text-sm">
                    <span className="font-medium text-slate-800">{g.grade}</span>
                    <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-0.5 text-xs tabular-nums text-slate-600">
                      <span>{g.count} students</span>
                      <span className="text-rose-600">{g.below40} below</span>
                      <span className="text-emerald-600">{g.above40} above</span>
                      <span className="font-semibold text-amber-700">{formatPct(g.averagePct)}</span>
                    </div>
                  </li>
                ))}
              </ul>
              <ResponsiveTable className="hidden rounded-xl border border-slate-200 lg:block">
                <table className="w-full min-w-[320px] border-collapse text-sm">
                  ...
                  <tbody>
                    {detail.gradeBreakdown.map((g) => (
                      <tr key={g.grade} className="bg-white">
                        <td className="border-b border-slate-100 px-3 py-2.5 font-medium last:border-b-0">
                          {g.grade}
                        </td>
                        <td className="border-b border-slate-100 px-3 py-2.5 text-right tabular-nums last:border-b-0">
                          {g.count}
                        </td>
                        <td className="border-b border-slate-100 px-3 py-2.5 text-right last:border-b-0">
                          <CountCell value={g.below40} variant="below" />
                        </td>
                        <td className="border-b border-slate-100 px-3 py-2.5 text-right last:border-b-0">
                          <CountCell value={g.above40} variant="above" />
                        </td>
                        <td className="border-b border-slate-100 px-3 py-2.5 text-right tabular-nums last:border-b-0">
                          {formatPct(g.averagePct)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div>
            ...
            <ResponsiveTable className="hidden rounded-xl border border-slate-200 lg:block">
              <table className="w-full min-w-[480px] border-collapse text-sm">
                ...
                <tbody>
                  {detail.students.map((s) => (
                    <tr key={`${s.admissionNumber}-${s.grade}`} className="bg-white">
                      ...
                      <td className="border-b border-slate-100 px-3 py-2.5 text-slate-600 last:border-b-0">
                        {s.grade}
                      </td>
