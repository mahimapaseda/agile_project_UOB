...
import { formatPersonName } from './person-name-display';
...
function matchesTeacherRow(
  ...
  const teacherName = formatPersonName(teacher?.name) || 'Unassigned';
...
export function buildGradeSubjectDetail(
  ...
  const teacher =
    (row.teacherStaffId
      ...
      : null) ??
    staffList.find((s) => formatPersonName(s.name) === row.teacherName) ??
    null;
...
export function buildTeacherSubjectAnalysis(
  ...
  for (const { subject, teacher, pcts } of map.values()) {
    ...
    rows.push({
      subject,
      teacherName: formatPersonName(teacher?.name) || 'Unassigned',
